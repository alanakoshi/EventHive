...
function Home() {
  ...
  const today = new Date();
    ...
    // subscribe to auth state, then load events
    useEffect(() => {
      let intervalId;
  ...
  // 2) Sort all by date (undated last)
  ...
  // 3) Derive lists...
  const upcoming   = useMemo(() => allSorted.filter(e => e.rawDate && new Date(e.rawDate) >= today), [allSorted, today]);
  const past       = useMemo(() => allSorted.filter(e => e.rawDate && new Date(e.rawDate) <  today), [allSorted, today]);
  const undated    = useMemo(() => allSorted.filter(e => !e.rawDate), [allSorted]);
  ...
  // 4) Build groupings
  const groupedAll      = useMemo(() => groupBy(allSorted,      ev => monthYear(ev.rawDate)), [allSorted]);
  const groupedUpcoming = useMemo(() => groupBy(upcoming,       ev => monthYear(ev.rawDate)), [upcoming]);
  const groupedPast     = useMemo(() => groupBy(past,           ev => monthYear(ev.rawDate)), [past]);
  const groupedUndated  = { 'No Date Yet': undated };
  const groupedUnfin    = useMemo(() => groupBy(unfinished,     ev => monthYear(ev.rawDate)), [unfinished]);
  ...
  const pastel = ['#ffd1dc','#ffecd1','#c1f0f6','#e0bbf9','#d0f0c0','#fdfd96','#aec6cf','#fbc4ab','#caffbf','#a0c4ff'];
      ...
      <div className="events-list">
        {Object.entries(groups).map(([label, evs])=>(
          <React.Fragment key={label}>
            <h2>{label}</h2>
            {evs.map(ev => {
              // if a theme was voted top, use its image, else fallback:
